...
<div class="scorebox"><strong>Total partie</strong>Vous {game_score[0]} · Eux {game_score[1]}{_game_winner_html(session.game)}</div>
...
def _game_winner_html(game: GameState) -> str:
    winner = game.winning_team()
    if winner is None:
        return ""
    label = "Vous gagnez" if winner == Team.TEAM_0_2 else "Eux gagnent"
    return f"<br><small>Partie terminée: {label}</small>"
