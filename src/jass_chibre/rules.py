...
def legal_cards(hand: list[Card] | tuple[Card, ...], current_trick: list[Card] | tuple[Card, ...], trump: Suit) -> tuple[Card, ...]:
    ...
    Interprétation ajustée pour la variante testée dans l'interface: quand une
    couleur non-atout est demandée, le joueur peut fournir ou couper même s'il
    possède la couleur demandée. La sous-coupe reste interdite, sauf si elle est
    ...
    if lead_suit == trump:
        trump_cards = tuple(card for card in hand_tuple if card.suit == trump)
    ...
    if strongest_trump_in_trick is None:
        if non_bour_follow_cards:
            return non_bour_follow_cards + trump_cards
        return hand_tuple
    ...
    if non_bour_follow_cards:
        candidates = non_bour_follow_cards + trump_cards
    else:
        candidates = hand_tuple
    not_undertrumps = tuple(card for card in candidates if card not in undertrumps)
