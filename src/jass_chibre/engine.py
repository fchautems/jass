...
class GameState:
    ...
    def winning_team(self) -> Team | None:
        """Retourne l'équipe gagnante si la cible est atteinte et départagée.

        Les points d'une donne sont ajoutés d'un bloc. Si les deux équipes
        dépassent la cible sur la même donne, l'équipe au total le plus haut est
        considérée gagnante. Une égalité exacte à la cible ou au-dessus reste
        non départagée et retourne ``None``.
        """
        if not self.is_game_over():
            return None
        score_02 = self.score[Team.TEAM_0_2]
        score_13 = self.score[Team.TEAM_1_3]
        if score_02 == score_13:
            return None
        return Team.TEAM_0_2 if score_02 > score_13 else Team.TEAM_1_3
