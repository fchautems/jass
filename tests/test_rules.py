...
def test_match_scores_257_without_announcements_or_stoeck():
    deal = DealState(hands={0: [], 1: [], 2: [], 3: []}, dealer_starter=0, chooser=0, trump=Suit.HEARTS)
    deal.trick_points_by_team[Team.TEAM_0_2] = 157
    deal.trick_wins_by_team[Team.TEAM_0_2] = 9

    deal._finish_deal()

    assert deal.deal_points_by_team[Team.TEAM_0_2] == 257
    assert deal.deal_points_by_team[Team.TEAM_1_3] == 0


def test_game_over_at_1000_reports_winning_team_after_finished_deal():
    game = GameState(options=GameOptions.normal())
    game.score[Team.TEAM_0_2] = 930
    game.score[Team.TEAM_1_3] = 980
    deal = DealState(hands={0: [], 1: [], 2: [], 3: []}, dealer_starter=0, finished=True)
    deal.deal_points_by_team[Team.TEAM_0_2] = 80
    deal.deal_points_by_team[Team.TEAM_1_3] = 20
    game.current_deal = deal

    game.finish_deal_if_done()

    assert game.score_tuple() == (1010, 1000)
    assert game.is_game_over()
    assert game.winning_team() == Team.TEAM_0_2
