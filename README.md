...
- validation des plis, fourniture, coupe possible avec atout, défausse et sous-coupe interdite ;
- exception du bour : le valet d'atout n'est jamais forcé ;
- points de plis, dernier pli à +5 et match à +100 ;
- fin de partie à 1000 points en mode normal, avec détection de l'équipe gagnante ;
...
2. **Coupe même avec la couleur demandée** : l'interface et le moteur autorisent maintenant de couper avec un atout même si le joueur possède aussi la couleur demandée. La sous-coupe reste interdite sauf si elle est le seul choix possible.
3. **Bour jamais forcé** : le moteur exclut le bour des obligations de fournir/couper. Si l'atout est demandé et que le joueur ne possède que le bour comme atout, il peut jouer une autre carte de sa main.
4. **Pique double** : l'option double actuellement l'ensemble des points de la donne calculés par le moteur, y compris plis, match, annonces ordinaires et stöck. Si la variante pratiquée ne double qu'une partie de ces points, il faudra ajuster cette option.
...
- voir quand la partie est terminée et quelle équipe a gagné après avoir atteint 1000 points.
